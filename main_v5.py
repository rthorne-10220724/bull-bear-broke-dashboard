...
# A BUY order is placed slightly above the latest price to improve fill odds.
...
try:
    ...
    # 60 days gives sufficient depth for EMA 50.
    ...
    # Avoid using the currently forming 4H candle.
    if len(df_4h) > 1:
        df_4h = df_4h.iloc[:-1]
...
# Detect an actual recovery from oversold territory.
rsi_reversal = (
...
1. QQQ higher timeframe must be bearish.
2. QQQ must show actual weakness/reversal downward.
3. SQQQ itself must show bullish confirmation.

We do NOT artificially invert the RSI.
"""
...
# SQQQ itself should be strengthening.
if not (
...
```
"""
Halts trading and force-liquidates open positions
when daily drawdown reaches configured maximum.
"""
...
try:
    ...
    # Fail safe: do not automatically halt because
    # of a temporary risk-check API failure.
...
```
"""
Returns symbols with open/pending orders.

This prevents the bot from submitting repeated
orders every 60 seconds when an existing limit
...
try:
    ...
    for order in orders:
        ...
        # Parent and child bracket orders can appear
        # depending on API behavior. Symbol-level
...
```
"""
Slightly marketable limit price.

This improves fill probability while retaining
a maximum entry price.
"""
...
for ticker in TARGETS:
    ...
    # Recheck capacity in case earlier loop entry
    ...
    # -----------------------------------------------------------
        ...
        # TQQQ direction is governed by QQQ's
    ...
    # -----------------------------------------------------------
    # Final order protection check
    #
    # Re-fetch immediately before submission to
    # reduce race conditions / duplicate orders.
    # -----------------------------------------------------------
    ...
    if response is not None:
        ...
        # Short pause before checking additional
        # symbols so broker state can update.
        time.sleep(2)
```
